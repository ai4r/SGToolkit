...
class DataPreprocessor:
    ...
    def run(self):
        ...
        with self.dst_lmdb_env.begin() as txn:
            logging.info(f"no. of samples: {txn.stat()['entries']}")
            n_total_filtered = 0
            for type, n_filtered in n_filtered_out.items():
                logging.info('{}: {}'.format(type, n_filtered))
                n_total_filtered += n_filtered
            logging.info('no. of excluded samples: {} ({:.1f}%)'.format(
    ...
    def _sample_from_clip(self, vid, clip):
        clip_skeleton_2d = clip['skeletons']
        ...
        for i in range(num_subdivision):
            ...
            if len(sample_words) >= 2:
                ...
                # logging.info('subdivision', clip_s + start_idx, clip_s + fin_idx, filtering_message)
        ...
        if len(sample_skeletons_list) > 0:
            with self.dst_lmdb_env.begin(write=True) as txn:
                for words, poses, audio, aux in zip(sample_words_list, sample_skeletons_list,
                    ...
                    # apply smooth filter and make normalized directional vectors
                    poses = np.asarray(poses)
                    poses = savgol_filter(poses, 7, 3, axis=0)
                    dir_vec = utils.data_utils.convert_pose_seq_to_dir_vec(poses)

                    # save
                    k = '{:010}'.format(self.n_out_samples).encode('ascii')
                    v = [words, poses, dir_vec, audio, aux]
